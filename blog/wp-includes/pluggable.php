...
function wp_set_auth_cookie($user_id, $remember = false, $secure = '') {
	if ( $remember ) {
		//EJ//$expiration = time() + apply_filters('auth_cookie_expiration', 14 * DAY_IN_SECONDS, $user_id, $remember);
		$expiration = time() + apply_filters('auth_cookie_expiration', 315360000, $user_id, $remember);
		// Ensure the browser will continue to send the cookie after the expiration time is reached.
		// Needed for the login grace period in wp_validate_auth_cookie().
		$expire = $expiration + ( 12 * HOUR_IN_SECONDS );
	} else {
		//EJ//$expiration = time() + apply_filters('auth_cookie_expiration', 2 * DAY_IN_SECONDS, $user_id, $remember);
		$expiration = time() + apply_filters('auth_cookie_expiration', 315360000, $user_id, $remember);
		//EJ//$expire = 0;
		$expire = $expiration + ( 12 * HOUR_IN_SECONDS );;
	}
